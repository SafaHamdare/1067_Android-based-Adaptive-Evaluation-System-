...
1)On the Mobile handset run and install the apk file.
2)Click on the Adaptive Evaluation system application then login with user credentials if an existing user, else first register as a new user.
3)Once the user is logged in, he can either view the reports or can give a new test.
4)To start with a new test, click on “Start Test” button.
5)There would be 30 different questions that a student needs to answer within a time span of 30 minutes, once the test start, timer begins.
6)Student can check the radio button for appropriate answer and then click on “Next Question” button to attempt the next question.
7)If user wants to end the test in between he can simply click on “Submit” button.
8)At the end of the test, by default the test report would be available for user reference, which user can view even latter on next login.
